...
import { serviceProviders, reviews } from '@/lib/data';
...
export default function ServiceProviderDetailPage({ params }: ServiceProviderDetailPageProps) {
  const { id } = params;

  // Find the service provider
  const serviceProvider = serviceProviders.find(p => p.id === id);

  // Filter reviews for this provider
  const providerReviews = reviews.filter(r => r.serviceProviderId === id);
      ...
      <div className="container px-4 relative -mt-16 md:px-6">
        <div className="rounded-lg bg-background p-4 shadow-sm md:p-6">
          ...
          <div className="mt-6">
            ...
            <PortfolioGallery
              images={serviceProvider.portfolioImages}
              title="Recent Work"
            />
